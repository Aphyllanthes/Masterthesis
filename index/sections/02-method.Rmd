...
Das zweite Modell (siehe Gleichung \ref{eq2}) entsteht durch schrittweise rückwärtsgerichtete Modellselektion des ersten Modells anhand des AIC. Hierzu wird die Funktion $stepAIC()$  aus dem Paket $MASS$ verwendet.
...
Die Modellergebnisse wurden als Effektplots mithilfe der Funktion $plot_model()$ aus dem Paket $sjPlot$ dargestellt um den Einfluss mehrerer Prädiktoren zu zeigen. 
Zur Darstellung der Modellergebnisse von Artenzahl und Abundanz der Wildbienen wurden Modellsimulationen mit der Funktion $simulate()$ aus dem Paket $stats$ durchgeführt. Die aus den Simulationen berechneten Werte des Mittelwert und der 95% Quantile wurden visualisiert.

#### Modellierung der Artenzahl:
Zur Analyse des Einflusses des Versiegelungsgrades verschiedener Radien auf die Artenzahl wurde für jeden Radius ein Modell nach der oben beschriebenen Methode zur Modellselektion ausgewählt. Für alle Radien des Versiegelungsgrades zeigte das durch die schrittweise Modellselektion (siehe Gleichung \ref{eq2}) erzeugte Modell den besten Fit. Die Modellzusammenfassung ist in Tabelle \ref{summary_Artenzahl} dargestellt. Die Modellformeln sind im folgenden aufgeführt, wobei "imp_r" den Versiegelungsgrad im Radius r beschreibt, "Temp." die mittlere Jahrestempertatur in Grad Celsius, "Höhe" die Höhenlage über NN und "Weideflächen" den Anteil an Weideflächen im 500m-Radius:
