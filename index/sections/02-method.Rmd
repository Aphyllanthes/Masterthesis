...
---
# Methoden
In dieser Arbeit wurden Bienen und ihre Gegenspieler aus Nishilfen bestimmt, Daten zur Landschaftsstruktur und Urbanisierung anhnand von Satellitendaten ermittelt und Zusammenhänge des Bienenvorkommens mittels glm (generalisieren linearen Modellen) modelliert.
<!-- package BAT, nlme -->
...
## Datengrundlage
### Bienen und natürlichen Gegenspieler
...
(ref:karte1) Die Karte zeigt ein Beispielausschnitt mit drei Nisthilfenstandorten in Freiburg. Für jeden der abgebildeten Buffer mit den Radien 100m,  250m, 500m, 1000m und 2000m um die Nisthilfenstandorte wurde der Mittelewert der versiegelten Fläche berechnet. Hierzu wurden die dargestellten Rasterdaten des Versiegelungsgrades verwendet. Pixel mit dem Wert Null sind in der Abbildung transparent dargestellt, sodass die Openstreetmap-Basiskarte dort sichtbar ist wo die Fläche unversiegelt ist. \label{karte}
...
```{r, echo=FALSE, out.width=c('90%'), fig.asp = 1, fig.show='hold', fig.cap = "(ref:karte1) ", fig.scap="Beispielkarte des Versiegelungsgrad als Datenbasis für den Urbanisierungsgrad"}
knitr::include_graphics("figures/Beispieldarstellung.png")
```
 
```{r, echo=FALSE, out.width=c('50%'), fig.asp = 1, fig.show='hold', fig.cap = "(ref:karte2) ", fig.scap="Beispielkarte der Landnutzungsklassen als Datenbasis für die Landschaftskomplexität"}
...
Zunchächst wurde der Einfluss von vorhandenen Insektenhäusern am Nisthilfenstandort analysiert. Da die Information über Insektenhäuser am Nisthilfenstandort nur für einen Teil (129 von 259) der Nisthilfen zur Verfügung stand, wurden folgende Modelle mit reduziertem Datensatz gefittet: 
...
Im drittes Modell (siehe Gleichung \ref{eq3}) wird der Versiegelungsgrad als quadratischer sowie als linearer Term implementiert, im vierten Modell (siehe Gleichung \ref{eq4}) ist der Versiegelungsgrad der einzige Prädiktor.  
