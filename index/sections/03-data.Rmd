...
# Ergebnisse
In den Nisthilfen des Schulinsektenhaus-Projektes, welche im Jahr 2019 ausgehängt waren, wurden 4716 Bienennester sowie 2147 Wespennester aufgefunden. Somit machen die in dieser Arbeit behandelten Nester etwa 70 % der vorgefundenen Nester aus. 
Alle weiteren Ergebnisse beziehen sich ausschließlich auf die Bienennester. 
...
Von den insgesamt `r round(sum(nests$nb_cells, na.rm = T),0)` angelegten Zellen entwickelten sich in etwa 60 % (`r round(sum(nests$nb_pupa_larva, na.rm = T),0)` Brutzellen) Puppen bzw. Larven. Etwa 13 % der angelegten Zellen wurden parasitiert (`r sum(nests$parasitized_cells, na.rm = T)` Brutzellen).  
In 45 der 259 Nisthilfen kamen keine Bienen vor.  
Im Mittel waren zwei Bienenarten in einer Nisthilfe, die maximale Bienenartenzahl in einer Nisthilfe betrug sieben Arten. Der Mittelwert der Abundanz betrug 72 Brutzellen pro Nisthilfe, 844 Brutzellen waren das Maximum an Brutzellen in einer Nisthilfe. Histogramme von Artenzahl und Abundanz der Wildbienen sind im Anhang \@ref(app2) in Abbildung \ref{deskriptiv} dargestellt.
Die häufigste Art in den Nisthilfen war die Rostrote Mauerbiene (*Osmia bicornis*). Sie kam in 148 der 259 Nisthilfen vor. Im Mittel befanden sich 8,4 Nester in einer Nisthilfe, der Median lag bei 2 belegten Nestern pro Nisthilfe. Die maximale Anzahl an Nestern in einer Nisthilfe betrug 115 Nester. Die Brutzellen von *O. bicornis* machten 40% der gesamten Brutzellen (Abundanz der Wildbienen) aus und die Nester der Art machten 46 % aller belegten Nester aus.  
...
Die Artengemeinschaft von Standorten, bei denen im 500m-Radius Ackerland vorkommt, unterschied sich nicht von der Artengemeinschaft, bei der im 500m-Radius kein Ackerland vorkommt.    


(ref:lonelytab) Auflistung der Arten, welche nur in einer Nisthilfe vorkamen, mit der Information wie viele Nester in der Nisthilfe belegt waren ('Nester') und dem mittleren Versiegelungsgrad in % im 500m-Radius der besiedelten Nisthilfe ('imp 500'). \label{lonely_species} 


```{r echo=FALSE, message=FALSE, warning=FALSE}
# Arten die nur in einem Nest vorkamen:
lonely_species <- read.csv("../summaryTabellen/lonely_species_summary.csv") %>% 
  mutate(imp500 = round(imp500)) %>% 
...
lonely_species %>%
  kable(booktabs = T, linesep = "", longtable =T, 
        ...
        caption.short = "Arten, welche nur in einer Nisthilfe vorkamen und der Versiegelungsgrad") %>%
...
Unterschiede bezüglich des Versiegelungsgrades konnten anhand der Betrachtung der einzelnen Arten aufgezeigt werden.
...
land_species[,1:5] %>% 
  kable(booktabs = T, linesep = "", longtable =T, 
        ...
        caption.short = "Arten mit Urbanisierungskategorie ihrer Nisthilfestandorte" ) %>% #
  kable_paper(full_width = F) %>%
...
## Der Einfluss des Versiegelungsgrades
...
Dies zeigte der Vergleich der Modelle, die den mittleren Versiegelungsgrad als beschreibende Variable von jeweils verschiedenen Radien enthielten. Der AIC-Wert des für jeden Radius anhand des besten Fit ausgewählten Modells war von den Modellen der anderen Radien jeweils um weniger als zwei Einheiten unterschiedlich.   
Den niedrigsten AIC wiesen das Modell, welches den Versiegelungsgrad im 100m-Radius beinhaltet, sowie das Modell, welches den Versiegelungsgrad im 1000m-Radius beinhaltet, auf.
...
Für die Versiegelung in größerer Entfernung um den Nisthilfenstandort, welche vielmehr den generellen Grad der Urbanisierung wiederspiegelt als die direkte Ressourcenverfügbarkeit für die Bienen, zeigt einen signifikanten quadratischen Einfluss auf die Artenzahl ($p = 0.006$). Bei einem Versiegelungsgrad von ca. $30 \%$ im 1000m-Radius ist die Artenzahl maximal (Abbildung \ref{plot_modelle_artenzahl} rechts) und nimmt bis zur maximalen Versiegelung ab, wo die Artenzahl den niedrigsten Wert aufweist. Dieser Effekt zeigt sich auch durch den Einfluss des Versiegelungsgrades im 2000m-Radius auf die Artenzahl ($p = 0.006$).   
Alle ausgewählten Modelle beinhalten die weiteren Prädiktoren $Jahresniederschlag$ und $Jahresmitteltemperatur$. Der Jahresniederschlag zeigt jeweils einen schwach signifikanten negativen Einfluss auf die Artenzahl, die Jahresmitteltemperatur einen schwach signifikanten positiven Einfluss auf die Artenzahl.
...
kableExtra::kbl(model_Artenzahl[-c(1,2,10),], booktabs =T, caption = "Modellzusammenfassung der ausgewählten Modelle der Artenzahl der Wildbienen, für jeden Radius des Versiegelungsgrades (imp). Die Modelle wurden jeweils unter der Annahme der Negativbinomialverteilung gefittet.  \\label{summary_Artenzahl} " , caption.short = "Modellzusammenfassung der Modelle der Artenzahl der Wildbienen ") %>% 
...
gridExtra::grid.arrange(p1a, p2a, ncol = 2, heights = unit(3.3, c("in")))
```

#### Die Abundanz der Bienen in Abhängigkeit des Versiegelungsgrades:
Der Einfluss des Versiegelungsgrades auf die Abundanz der Wildbienen in den Nisthilfen war, wie auch der Einfluss auf die Artenzahl, für den Versiegelungsgrad geringer Radien (100m, 250, 500m) linear und für den Versiegelungsgrad großer Radien (500m, 1000m, 2000m) um die Nisthilfen quadratisch. Im Vergleich zur Artenzahl eigneten sich zur Beschreibung der Abundanz jedoch nicht alle Radien des Versiegelungsgrades gleichermaßen. Die Modelle des Versiegelungsgrades im 250m-Radius ($AIC = 2573.81$), sowie im 500m-Radius ($AIC = 2575.67$) zeigten den besten Fit. Die Modellzusammenfassung der beiden Modelle ist in Tabelle \ref{summary_Abundanz_a} dargestellt.  
Der mittlere Versiegelungsgrad im 250m-Radius hat mit einem p-Wert von $0,00003$ einen hochsignifikanten linearen Einfluss auf die Abundanz (siehe Abbildung \ref{plot_modelle_abundanz}). Das Modell zeigt auch einen signifikanten positiven Effekt der Anzahl an Landnutzungsklassen auf, sowie einen signifikanten negativen Effekt der Kantenlänge, welcher in Abschnitt \@ref(landnutzung) genauer erläutert wird. Der quadratische Einfluss des Versiegelungsgrades auf die Abundanz, welcher vom Versiegelungsgrad des 500m-Radius beschrieben wird, zeigt das Maximum der Abundanz bei etwa $25\%$ (siehe Abbildung \ref{plot_modelle_abundanz}).
...
kableExtra::kbl(model_abundanz_a[,-17], booktabs =T, 
                ...
                caption.short="Modellzusammenfassung der Modelle der Abundanz der Wildbienen") %>% 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Die Modelle des mittleren Versiegelungsgrades im 250m-Radius (links) sowie 500m-Radius (rechts), welche den Einfluss der Nullen (Zeroinflation) berücksichtigen. Die Linie beschreibt den Mittelwert der anhand der R-Funktion \\textit{simulate()} simulierten Daten, der violett gezeichnete Bereich beschreibt den Bereich bis zum 95 \\% Quantil der simulierten Daten, die Punkte sind die realen Messwerte. \\label{plot_modelle_abundanz}", fig.scap="Einfluss des Versiegelungsgrades auf die Abundanz der Bienen"}
...
#### Die Abundanz von *Osmia bicornis* in Abhängigkeit des Versiegelungsgrades:

(ref:modellsOb) Modellzusammenfassung der ausgewählten Modelle von *Osmia bicornis*, für jeden Radius des Versiegelungsgrades. Die Modelle wurden jeweils unter der Annahme der Negativbinomialverteilung gefittet.  \label{summary_Abundanz_ob} 


(ref:modellsObscap) Modellzusammenfassung der Modelle der Abundanz von *Osmia bicornis*
...
kableExtra::kbl(model_ob1[-c(1,2),], booktabs =T, caption = "(ref:modellsOb)",
                caption.short = "(ref:modellsObscap)") %>% 
  kableExtra::add_header_above(
    c(" ", "imp100"=3,"imp250"=3,"imp500"=3, "imp1000"=3, "imp2000"=3) 
  ) %>%
  kableExtra::kable_styling(latex_options =c("repeat_header", "scale_down", "striped"), repeat_header_text = "(ref:continued)") %>% 
...
Dieses Modell kann durch die Erweiterung um die Prädiktoren des mittleren Versiegelungsgrades als linearen sowie quadratischen Term nicht verbessert werden. Dies gilt für alle Radien, für die der mittlere Versiegelungsgrad berechnet wurde. Der AIC dieser Modelle im Vergleich zum einfacheren Modell ohne den Versiegelungsgrad als Prädiktor variiert jeweils um nicht mehr als zwei Einheiten.    
...
#### Abundanz der Wildbienen in Abhängigkeit bestehender Insektenhäuser:
Der Mittelwert der Abundanz der Wildbienen in den Nisthilfen liegt bei 89,7 Brutzellen pro Nisthilfe, sofern im Umfeld der Nisthilfe ein Insektenhaus vorhanden war, und bei 66,4 Brutzellen pro Nisthilfe, sofern kein Insektenhaus in der Nähe der Nisthilfe vorhanden war. Die Modellierung der Abundanz anhand der Information von vorhandenen Insektenhäusern zeigt jedoch keinen signifikanten Einfluss ($p-Wert = 0.45$) des Insektenhauses auf die Abundanz auf (siehe Tab. \ref{summaryIH}). Das Modell enthält zusätzlich den mittleren Versiegelungsgrad im 250m-Radius als linearen und quadratischen Term.  
Nur der Versiegelungsgrad im 250 m Radius verbessert das Model. Behält man den Versiegelungsgrad im Modell, so verbessert das “Insektenhaus” das Modell nicht. Dies gilt für alle Radien.  
Die Erweiterung des Modells um den Versiegelungsgrad im 250m-Radius ist der einzige Radius des Versiegelungsgrades, welcher das Modell der bestehenden Insektenhäusern verbessert. Andersherum kann durch den Parameter "Insektenhaus" kein Modell, welches den Einfluss des Versiegelungsgrad der verschiedenen Radien beschreibt, verbessert werden. 
Ein Effekt-Plot des um den Parameter des 250m-Versiegelungsgrades erweiterten Modells ist in Abbilung \ref{effektplotIH} unten links dargestellt. Das Modell zeigt einen negativen signifikanten linearen Zusammenhang zwischen dem mittleren Versiegelungsgrad im 250m-Radius und der Abundanz (siehe Tab. \ref{summaryIH}).



#### Abundanz von *Osmia bicornis* in Abhängigkeit bestehender Insektenhäuser:
...
(ref:effektIH) Einfluss von vorhandenem Insektenhaus auf Artenanzahl der Bienen (oben), Abundanz der Bienen (links unten) und Abundanz von *Osmia bicornis* (recht unten) . Die Fehlerbalken sind 'marginal effect-plots', welche die Vorhersage der Modelle (Mittelwert und Standartabweichung) darstellen. Die Effektplots der Abundanzen beinhalten den skalierten Versiegelungsgrad im 250m-Radius ("imp 250"). Dargestellt ist der mittlere Versiegelungsgrad von 39.08% (skalierter Wert = 0) und der mittlere Versiegelungsgrad +- Standartabweichung 18,37% (skalierter Wert = -1) und 59,8% (skalierter Wert = 1). Die Punkte stellen die Artenzahl/Abundanz der einzelnen Nisthilfen dar. \label{effektplotIH}

(ref:effektIH2) Einfluss von bestehenden Insektenhäusern auf Artenanzahl und Abundanz der Wildbienen und Abundanz von *Osmia bicornis*

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "(ref:effektIH)", fig.scap="(ref:effektIH2)", fig.height=5.8 }
library(MASS)
...
gridExtra::grid.arrange(p1ep, p2pm, p3pm,
                        heights=c(grid::unit(7, "in"), grid::unit(7.8, "in")),
                        layout_matrix = hmat)
...
table_a[,1] <- c("(Interzept)", "Insektenhaus", "imp250", "imp250^2", "Breitengrad", "Zählmodell: Interzept", "Zeroinflation: Interzept")
kableExtra::kbl(table_a, booktabs =T, caption = "Modellzusammenfassung der Modelle zum Einfluss von vorhandenen Insektenhäusern auf Artenzahl und Abundanz der Bienen und Abundanz von Osmia bicornis. Die Modelle wurden jeweils unter der Annahme der Negativbinomialverteilung der Antwortvariablen an den reduzierten Datensatz (129 Beobachtungen) aller Nisthilfen mit Information zum Vorhandensein eines Insektenhauses gefittet.  \\label{summaryIH} " , caption.short = "Modellzusammenfassung: Einfluss von vorhandenen Insektenhäusern auf Artenzahl und Abundanz", col.names = c('Predictors', 'Log-Mean', 'CI', 'p', 'Log-Mean', 'CI', 'p', 'Log-Mean', 'CI', 'p') ) %>% #caption = "" 
  kable_styling(latex_options =c("striped")) %>% 
...
Der Einfluss der Artenzahl der Wildbienen auf die Artenzahl der natürlichen Gegenspieler ist hochsignifikant ($p<2e-16$). Ein zusätzlicher Einfluss von Versiegelungsgrad und vorhandenen Insektenhäusern am Nisthilfestandort auf die Artenzahl der natürlichen Gegenspieler kann in dieser Arbeit nicht aufgezeigt werden.   Die Modellannahmen der Modelle, welche den Einfluss des Versiegelungsgrades auf die Artenzahl der natürlichen Gegenspieler beschreiben, sind durch die Devianz der Quantile verletzt. 
Die Modellzusammenfassung des Modells, welches den Einfluss der Artenzahl der Wildbienen auf die der Gegenspieler aufzeigt, ist im Anhang \@ref(app2) in Tabelle \ref{tabGegenspieler} abgebildet.
...
Die Netzwerkanalyse der Wirt-Parasitoid-Beziehungen der Kategorie "Land", welche 107 Nisthilfestandorte  beinhaltet und der Kategorie "Stadt" welche 110 Nisthilfestandorte beinhaltest, ergibt für beide Kategorien mit dem Index H2' einen mittleren Spezialisierungsgrad von $0.60$ bzw. $0.61$. Alle Werte der Generalisierung der natürlichen Gegenspieler als auch der Vulnerabilität der Wirtsbienenarten beider Gruppen liegen zwischen $2.0$ und $2.5$. Somit sind keine deutlichen Unterschiede der Wirt-Parasit-Beziehungen zwischen ländlichen Nisthilfestandorten und urbanen Nisthilfestandorten zu erkennen. 
Die Netzwerkdarstellung der Kategorie "Land" (siehe Abbildung \ref{networkland}) wirkt komplexer als die Netzwerkdarstellung der Kategorie "Stadt" (siehe Abbildung \ref{networkstadt}). Dies bedeutet jedoch nicht, dass sich der Spezialisierungsgrad unterscheidet, sondern lediglich, dass es insgesamt mehr Arten und somit auch mehr Interaktionen gibt.


```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Das bipartite Interaktionsnetzwerk der Nisthilfen der Kategorie Land. Die Kategorie beinhaltet alle Nisthilfen mit einem mittleren Versiegelungsgrad im 500m-Radius von weniger als 24\\% versiegelter Fläche. Die oberen Balken stellen die natürlichen Gegenspieler dar, die unteren Balken deren Wirte. Die Interaktionen sind durch die Verbindung der oberen und unteren Balken dargestellt. Die Balkendicke stellt die Häufigkeit der Interaktionen dar, wobei diese pro Nest gezählt wurden.  \\label{networkland}", fig.scap="Wirt-Parasitoid-Netzwerk der Nisthilfen der Kategorie Land" }
...
plotweb(nw_frame[[1]], text.rot=90, y.lim = c(-3,5), ybig = 3)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Das bipartite Interaktionsnetzwerk der Nisthilfen der Kategorie Stadt. Die Kategorie beinhaltet alle Nisthilfen mit einem mittleren Versiegelungsgrad im 500m-Radius von mehr als 44\\% versiegelter Fläche. Die oberen Balken stellen die natürlichen Gegenspieler dar, die unteren Balken deren Wirte. Die Interaktionen sind durch die Verbindung der oberen und unteren Balken dargestellt. Die Balkendicke stellt die Häufigkeit der Interaktionen dar, wobei diese pro Nest gezählt wurden. \\label{networkstadt}", fig.scap="Wirt-Parasitoid-Netzwerk der Nisthilfen der Kategorie Stadt"}
plotweb(nw_frame[[3]],  text.rot=90, y.lim = c(-3,5), ybig = 3)
```
